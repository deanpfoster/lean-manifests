...
/-! # LeanManifests/Workplan.lean — Reusable workplan generator
...
(see `LeanManifests/Attr.lean` for definitions):
...
def main : IO Unit := LeanManifests.Workplan.run "YourProject"
```
...
namespace LeanManifests.Workplan
