...
### 5f. The semi-pivot: structural proofs over fixture proofs

Two production Lean software projects we audited as prior art:

  * **lean-zip** (Kim Morrison) — zlib bindings, **359 theorems,
    33,000 LoC, zero `native_decide`, zero `partial def` in pure
    modules.**
  * **RadixExperiment** (Leo de Moura, AI-led, one weekend) —
    verified DSL, **52 theorems, 7,400 LoC, zero `native_decide`,
    zero `partial def` in pure modules.**

Their discipline isn't "use `ProvenTheorem` macros." They don't
have a manifest layer. Their discipline is:

  1. **Spec first.** Define the intended behavior as a relational
     predicate (e.g., big-step semantics) before writing the
     implementation. The relation IS the spec. The implementation
     is the executable. Soundness + completeness theorems prove
     they agree.

  2. **Structural proofs.** Induction, `simp`, `decide`, `grind`,
     `omega`, named lemmas. Not `native_decide`.

  3. **Total functions.** No `partial def` in code that proofs
     reference.

  4. **Examples teach.** Their agents imitated Mathlib and the
     core Lean DSL declarations. Their output looked like Mathlib.

The `examples-of-good-theorems.lean` file (in this directory) is
the canonical seed. **Read it before authoring manifest entries.**
It shows the relational-spec-vs-implementation pattern, the
determinism pattern, the optimization-correctness pattern, and the
exception list for when `native_decide` is genuinely the right
proof method.

#### When `native_decide` IS legitimate

There's exactly one case where `native_decide` belongs in a
`ProvenTheorem`: **exhaustive checking over a fixed, finite,
mechanically-generated structure.** Canonical example:
...
The call graph is generated by a build script. The theorem is
universally quantified. The proof is "check every entry"; that's
what `native_decide` does.

Anywhere else, `native_decide` is the dress-up signal. If you find
yourself reaching for it on a non-generated input, ask:

  * Could this be a `private theorem ... := by rfl` helper instead?
  * Could the universal version be proven by induction?
  * Is this actually a UnitTest pinning a specific incident?

The answer is one of those three. Not "ProvenTheorem proven by
`native_decide` on a fixture."

#### Audit: how many `native_decide` uses do you have?

```bash
grep -rE ':= by native_decide|^  native_decide$' --include='*.lean' .
```

If the count is large and most are NOT in generated/exhaustive
contexts, you have the dress-up problem. The fix is the workflow
described in `examples-of-good-theorems.lean`.
